...
class ReferDataset2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data.append(dict(image=resized_img, sam_img = sam_img, height=np.asarray(img).shape[0], width=np.asarray(img).shape[1],
                         file_name=this_img['file_name'], cat_name=this_cat_name, img_id=this_img_id,
                         coco_instance_gt=MaskAnns, coco_instance_gt_box=BoxAnns ,coco_instance_cat=cat_names))
...
class ReferDataset2_new(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return data, np.asarray(annot), tensor_embeddings, sentence_raw, sents_id



class ReferDataset3(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return np.asarray(annot), tensor_embeddings, sentence_raw



class ReferDataset4(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data.append(dict(image=resized_img, pil_img=img, height=np.asarray(img).shape[0], width=np.asarray(img).shape[1],
                         file_name=this_img['file_name'], cat_name=this_cat_name, img_id=this_img_id,
                         coco_instance_gt=MaskAnns, coco_instance_gt_box=BoxAnns ,coco_instance_cat=cat_names))
