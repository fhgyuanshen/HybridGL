...
# refer dataset
from data.dataset_phrasecut_gem import PhraseCutDataset
...
def main(args, Height, Width, Neg):
    ...
    dataset = PhraseCutDataset('test',preprocesser=preprocess)
    ...
    for i, data in enumerate(tbar):
        ...
        #     continue
        with torch.no_grad():
            tensor_img, target, sentence_raw = (data[0]['tensor_img'].to(device), data[0]['gt_masks'], data[0]['phrase'])
            ...
            for sentence, j in zip(sentence_raw, range(len(sentence_raw))):
                ...
                path2attn=f"./data/var/PhraseCut/attns_gem_test_openai_b16/"
